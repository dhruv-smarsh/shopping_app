    ...
    return (
        <>
            ...
            {products.map((data: any) => {
                return(
                    <>
                        <S.Container key={data.sku} sku={data.sku} tabIndex={1}>
                            {data.isFreeShipping && <S.Stopper>Free shipping</S.Stopper>}
                            <S.Image alt={data.title} />
                            <S.Title>{data.title}</S.Title>
                            <S.Price>
                                <S.Val>
                                    <small>{data.currencyFormat}</small>
                                    <b>{data.price}</b>
                                    {/*<span>{data.formattedPrice.substring(data.formattedPrice.length - 3)}</span>*/}
                                </S.Val>
                                <S.Installment>
                                    <span>or {data.installments} x</span>
                                    <b>
                                        {data.currencyFormat}
                            ...
                            <S.BuyButton onClick={() => {this.props.addToCart(data); console.log(this.props)}} tabIndex={-1}>
                                Add to cart
                            </S.BuyButton>
                        </S.Container>
                    </>
...
}), mapDispatchToProps /** no second argument */)(Product);
